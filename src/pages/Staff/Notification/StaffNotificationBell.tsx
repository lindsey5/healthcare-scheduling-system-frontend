...
import useGetStaffNotifications from "../../../hooks/staff-notification/use-get-staff-notifications.hook";
...
import useReadStaffNotification from "../../../hooks/staff-notification/use-read-staff-notification.hook";
import useReadAllStaffNotifications from "../../../hooks/staff-notification/use-read-all-staff-notifications.hook";

export default function StaffNotificationBell() {
    const readNotificationMutation = useReadStaffNotification();
    const readAllNotificationMutation = useReadAllStaffNotifications();
    ...
    const { data } = useGetStaffNotifications({
        page: pagination.pageIndex + 1,
        limit: pagination.pageSize,
    });

    const socket = useSocket({ namespace: "/staff-notification" });
        ...
        if (data) {
            ...
            setNotifications((prev) =>
                pagination.pageIndex === 0
                    ...
                    : [...prev, ...data.staffNotifications]
            );
        }

        if(socket) {
            socket.on("staff-notification", (notification: StaffNotification) => {
    ...
    const handleReadAll = () => {
        promiseToast(
            readAllNotificationMutation.mutateAsync(),
            "top-center",
            () => {
                setUnread(0);
    ...
    return (
        ...
        >
            ...
            <button
                ...
                className="relative flex items-center justify-center rounded-full border border-gray-300 p-2.5 transition hover:bg-green-50 cursor-pointer"
            >
                <Bell
                    size={22}
            ...
            {open && (
                <div className="absolute right-0 z-20 mt-3 w-[400px] overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl">
                    {/* Header */}
                    <div className="flex items-center justify-between border-b border-gray-300 bg-white px-5 py-4">
                        <div>
                    ...
                    {/* Notifications */}
                    <div className="max-h-[420px] space-y-3 overflow-y-auto bg-gray-50 p-3">
                                        ...
                                        <div className="flex items-start gap-3">
                                            ...
                                            <div className="flex-1">
                                                <div className="flex items-start justify-between gap-3">
                                                    ...
                                                    {!notification.isRead && (
                                                        <span className="mt-2 h-2.5 w-2.5 flex-shrink-0 rounded-full bg-green-600" />
                                                    )}
                                                </div>
